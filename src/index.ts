...
import {
  ...
  renderNewPlaylistInput,
  renderRenamePlaylistInput,
} from "./ui";
import {
  loadFavorites,
  isFavorite,
...
async function checkDep(cmd: string): Promise<boolean> {
  // Windows'ta 'where', diğerlerinde (macOS, Linux) 'which' kullan
  const whichCmd = process.platform === "win32" ? "where" : "which";

  const proc = Bun.spawn([whichCmd, cmd], {
    stderr: "ignore",
    stdout: "ignore",
  });
  ...
  fetchMix(fromId, 20)
    .then((tracks) => {
      const newTracks = tracks.filter(
        (t) => t.id !== fromId && !existingIds.has(t.id),
      );
...
async function onSearchInput(key: string) {
  ...
  if (key === "\r" || key === "\n") {
    ...
    try {
      ...
      renderSearch(
        "",
        "Hata oluştu. yt-dlp kurulu olduğundan emin ol.",
        favorites.length > 0,
        playlists.length > 0,
        searchMode,
      );
